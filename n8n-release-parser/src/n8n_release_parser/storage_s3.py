...
Implements :class:`~n8n_release_parser.storage.StorageBackend` using
``boto3``.  Only imported when :func:`~n8n_release_parser.storage.create_backend`
encounters an ``s3://`` URI, keeping ``boto3`` as a lazy dependency for
local-only usage.
"""
