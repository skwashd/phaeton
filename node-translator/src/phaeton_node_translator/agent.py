"""Core Strands Agent logic for AI-powered node-to-ASL translation."""
...
SYSTEM_PROMPT = """\
You are a specialist translator that converts n8n workflow nodes into AWS Step \
Functions ASL state definitions.

## Constraints
1. Output must be valid ASL JSON — each state must have a "Type" field with one \
of: Task, Pass, Choice, Wait, Succeed, Fail, Parallel, Map.
2. Use JSONata (not JSONPath) for all data transformations.
3. Use SSM Parameter Store for any credentials — never embed secrets in state \
definitions.
4. State names must be 1-128 characters, alphanumeric with spaces and hyphens.
5. Include proper error handling (Retry/Catch) for Task states that call AWS \
services.
6. Flag any uncertainty in the output via the "warnings" field.
"""
...
def translate_node(request: NodeTranslationRequest) -> NodeTranslationResponse:
    """
    ...
    try:
        ...
        response = NodeTranslationResponse.model_validate(parsed)
        ...
        if response.states:
            ...
            if validation_errors:
                ...
                return NodeTranslationResponse(
