...
class TestLambdaSynthSucceeds:
    ...
    def test_synth_produces_cloudformation(self, code_node_synth: SynthResult) -> None:
...
class TestLambdaSynthLambdaFunctions:
    ...
    def test_has_lambda_functions(self, code_node_synth: SynthResult) -> None:
    ...
    def test_lambda_has_tracing(self, code_node_synth: SynthResult) -> None:
    ...
    def test_lambda_has_dlq(self, code_node_synth: SynthResult) -> None:
    ...
    def test_lambda_has_iam_role(self, code_node_synth: SynthResult) -> None:
        ...
        for logical_id, resource in lambdas.items():
            props = resource.get("Properties", {})
            assert "Role" in props, f"Lambda {logical_id} missing Role property"


class TestLambdaSynthStateMachine:
    """Verify the state machine resource in the synthesized template."""

    def test_has_state_machine(self, code_node_synth: SynthResult) -> None:
    ...
    def test_state_machine_has_logging(self, code_node_synth: SynthResult) -> None:
    ...
    def test_state_machine_has_tracing(self, code_node_synth: SynthResult) -> None:
...
class TestLambdaSynthIAM:
    ...
    def test_has_lambda_execution_roles(self, code_node_synth: SynthResult) -> None:
        """Template must contain IAM Roles for Lambda functions."""
        roles = code_node_synth.workflow_template.find_resources("AWS::IAM::Role")
        lambda_roles = [
            r
        ...
        assert len(lambda_roles) >= 1, "No IAM Roles for Lambda functions found"


class TestLambdaSynthInfrastructure:
    """Verify supporting infrastructure resources."""

    def test_has_dead_letter_queue(self, code_node_synth: SynthResult) -> None:
        """Template must contain an SQS dead-letter queue."""
        queues = code_node_synth.workflow_template.find_resources("AWS::SQS::Queue")
        assert len(queues) >= 1, "No SQS queues found"

    def test_has_cloudwatch_alarms(self, code_node_synth: SynthResult) -> None:
    ...
    def test_has_failed_execution_event_rule(
        ...
        rules = code_node_synth.workflow_template.find_resources("AWS::Events::Rule")
        rule_str = str(rules)
