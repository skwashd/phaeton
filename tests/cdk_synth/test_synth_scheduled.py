...
class TestScheduledSynthStateMachine:
    ...
    def test_definition_contains_sqs_state(
        self, scheduled_synth: SynthResult
    ) -> None:
...
class TestScheduledSynthTriggers:
    ...
    def test_has_schedule_rule(
        self, scheduled_synth: SynthResult
    ) -> None:
        """Template must contain an EventBridge rule with a schedule."""
        rules = scheduled_synth.workflow_template.find_resources(
            "AWS::Events::Rule"
        )
        schedule_rules = {
            lid: r
...
class TestScheduledSynthIAM:
    ...
    def test_has_iam_policy(
        self, scheduled_synth: SynthResult
    ) -> None:
        """Template must contain an IAM Policy for the execution role."""
        policies = scheduled_synth.workflow_template.find_resources(
            "AWS::IAM::Policy"
        )
...
class TestScheduledSynthInfrastructure:
    ...
    def test_has_dead_letter_queue(
        ...
        queues = scheduled_synth.workflow_template.find_resources(
            "AWS::SQS::Queue"
        )
