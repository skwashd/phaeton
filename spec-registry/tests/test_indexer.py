...
from spec_registry.storage import LocalStorageBackend

from .conftest import (
    ACME_SWAGGER2_SPEC,
    WIDGET_OPENAPI3_SPEC,
    write_json_spec,
    write_yaml_spec,
)
...
class TestBuildSpecIndexSwagger2:
    """Tests for building a spec index from Swagger 2.0 files."""

    def test_build_spec_index_swagger2(self, tmp_path: Path) -> None:
        """Swagger 2.0 spec is correctly parsed and indexed."""
        write_json_spec(tmp_path, "acme.json", ACME_SWAGGER2_SPEC)
...
class TestBuildSpecIndexOpenapi3:
    """Tests for building a spec index from OpenAPI 3.x files."""

    def test_build_spec_index_openapi3(self, tmp_path: Path) -> None:
        """OpenAPI 3.x spec is correctly parsed and indexed."""
        write_json_spec(tmp_path, "widget.json", WIDGET_OPENAPI3_SPEC)
        index = build_spec_index(tmp_path)
...
class TestBuildSpecIndexMixed:
    """Tests for building a spec index from a mixed directory."""

    def test_build_spec_index_mixed_directory(self, tmp_path: Path) -> None:
        """Mixed Swagger 2 and OpenAPI 3 specs are indexed together."""
        write_json_spec(tmp_path, "acme.json", ACME_SWAGGER2_SPEC)
        write_json_spec(tmp_path, "widget.json", WIDGET_OPENAPI3_SPEC)
        index = build_spec_index(tmp_path)
...
class TestNormalizeBaseUrl:
    ...
    def test_normalize_base_url_various_formats(self) -> None:
        """Various URL formats are normalized consistently."""
...
class TestExtractResourceOperations:
    """Tests for endpoint extraction from spec paths."""

    def test_extract_resource_operations_from_spec(self) -> None:
        """Endpoints are extracted from Swagger 2 paths."""
...
class TestSaveLoadRoundtrip:
    """Tests for index serialization round-trip."""

    def test_save_load_roundtrip(self, tmp_path: Path) -> None:
        """Index survives a save/load round-trip."""
        specs_dir = tmp_path / "specs"
        specs_dir.mkdir()
        write_json_spec(specs_dir, "acme.json", ACME_SWAGGER2_SPEC)
    ...
    def test_save_load_backend_roundtrip(self, tmp_path: Path) -> None:
        """Index survives a save/load round-trip through a storage backend."""
        specs_dir = tmp_path / "specs"
        specs_dir.mkdir()
        write_json_spec(specs_dir, "acme.json", ACME_SWAGGER2_SPEC)
        ...
        save_index_to_backend(original, backend)

        loaded = load_index_from_backend(backend)
        assert loaded is not None
        assert len(loaded.entries) == len(original.entries)
        assert loaded.entries[0].service_name == original.entries[0].service_name

    def test_load_index_from_backend_missing(self, tmp_path: Path) -> None:
        """Loading from backend returns None when index does not exist."""
        backend = LocalStorageBackend(tmp_path / "empty")
        result = load_index_from_backend(backend)
        assert result is None


class TestHandlesMissingFields:
    """Tests for specs with minimal/missing fields."""

    def test_handles_missing_fields(self, tmp_path: Path) -> None:
        """Minimal spec with no servers/auth is indexed gracefully."""
        minimal_spec: dict = {
            "openapi": "3.0.0",
        ...
        write_json_spec(tmp_path, "bare.json", minimal_spec)
        index = build_spec_index(tmp_path)
...
class TestYamlSpecParsing:
    """Tests for YAML spec file parsing."""

    def test_yaml_spec_parsing(self, tmp_path: Path) -> None:
        """YAML spec files are parsed and indexed correctly."""
        write_yaml_spec(tmp_path, "widget.yaml", WIDGET_OPENAPI3_SPEC)
        index = build_spec_index(tmp_path)
    ...
    def test_yml_extension(self, tmp_path: Path) -> None:
        """Files with .yml extension are also parsed."""
        write_yaml_spec(tmp_path, "acme.yml", ACME_SWAGGER2_SPEC)
        index = build_spec_index(tmp_path)
